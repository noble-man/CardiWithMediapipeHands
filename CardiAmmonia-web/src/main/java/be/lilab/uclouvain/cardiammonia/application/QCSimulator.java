...
public class QCSimulator {
	...
	private static Logger logger = LoggerFactory.getLogger(QCSimulator.class);

	public static void main(String[] args) {
	    try {
		    logger.info("starting the QC simulator server...");
		    SimulationServerBuilder.SIMULATION_SPEED = 60;
		    OpcUaServer qcServer = SimulationServerBuilder.buildQCServer();
			CommunicationClient client = CommunicationClientFactory.get()
					.setOpcUaProtocol()
					.setServerUrl("127.0.0.1:"+Constants.QC_TCP_PORT)
					.setServerPath("/milo")
					.setRootNodeUrl(Constants.QC_ROOT_URL).build();
			client.connect();
			client.callMethod(Constants.QC_ROOT_URL,"Start","1");
